...
namespace vrt {
  ...
  //    toVRTFloat(..)      - VRTPacket.h
  //    fromVRTFloat(..)    - VRTPacket.h
...
#define BYTE_ORDER (host_order.value)
   ...
   *
   *    Convert to/from double: VRTMath.toDouble32(20, bits)  -> val
   *                            VRTMath.fromDouble32(20, val) -> bits
   *  </pre>
   ...
   *  integer and floating-point formats that can be used when performing computations.
   *  <table border='1'>
   *    <tr>
   *      <th>&nbsp;</th>
   *      <td colspan='2'>Convert to/from <tt>long</tt></td>
   *      <td colspan='2'>Convert to/from <tt>int</tt></td>
   *      <td colspan='2'>Convert to/from <tt>float</tt></td>
   *      <td colspan='2'>Convert to/from <tt>double</tt></td>
   *    </tr>
   *    <tr>
   *      <th>64-bit Fixed Point</th>
  ...
  namespace VRTMath {
    ...
    inline int64_t fromDouble64 (int32_t radixPoint, double val) {
      double divisor = (double)(__INT64_C(0x1) << radixPoint);
      return (int64_t)rint(val * divisor);
    }
    ...
    inline int32_t fromDouble32 (int32_t radixPoint, double val) {
      double divisor = (double)(__INT64_C(0x1) << radixPoint);
      return (int32_t)rint(val * divisor);
    }
    ...
    inline int16_t fromDouble16 (int32_t radixPoint, double val) {
      double divisor = (double)(__INT64_C(0x1) << radixPoint);
      return (int16_t)rint(val * divisor);
    }
    ...
    inline void packShort (void *ptr, int32_t off, int16_t val, ByteOrder rep=BIG_ENDIAN) {
      char *buf = (char*)ptr;
      *((int16_t*)(&buf[off])) = (rep == BYTE_ORDER)? val : __bswap_16(val);
    }
   ...
   /** Pack unsigned byte data into byte buffer.
    ...
    inline void packUShort (void *ptr, int32_t off, uint16_t val, ByteOrder rep=BIG_ENDIAN) {
      char *buf = (char*)ptr;
      *((uint16_t*)(&buf[off])) = (rep == BYTE_ORDER)? val : __bswap_16(val);
    }
   ...
   /** Pack float data into byte buffer.
     ...
     */
    int32_t lengthUTF8 (const wstring &val);

    /** Computes the length a UTF-8 string will need in a buffer.
     *  @param val The string
     *  @return The length required to hold the entire UTF-8 string.
     */
    int32_t lengthUTF8 (const string &val);
